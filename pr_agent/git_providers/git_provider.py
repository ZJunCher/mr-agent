...
class GitProvider(ABC):
    ...
    def get_pr_target_branch(self) -> str:
        """Return the reviewed target branch when the provider exposes one."""
        return ""

    def get_pr_target_branch_sha(self) -> str:
        """Resolve the current target-branch head SHA used to pin trusted repository policy."""
        return ""

    def get_file_content_at_ref(self, file_path: str, ref: str) -> str | None:
        """Read a repository text file at an exact ref; ``None`` means it does not exist."""
        return None
    ...
    def _possible_headers(self):
        return (
            ...
            "### 文件详解",
            "### **文件详解**",
            "<details> <summary><h3> 文件详解</h3></summary>",
            "<details open> <summary><h3> 文件详解</h3></summary>",
    ...
    def add_reaction(self, issue_comment_id: int, emoji_name: str) -> Optional[int]:
        """Add an award emoji reaction to a comment. Default no-op, override in providers."""
        return None
    ...
    def get_diff_refs(self) -> Optional[dict]:
        """Return the frozen diff refs (base_sha/head_sha/start_sha) if available.

        Used by the offline evaluation capture to freeze the exact code state a
        review was produced against, so it can later be replayed. Best-effort:
        providers that cannot expose these refs return None.
        """
        return None
