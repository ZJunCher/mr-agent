#!/usr/bin/env python3
"""Detect and unlock MRs stuck on the feedback-gate commit status.

The mandatory feedback gate marks an MR's head commit with a ``pending`` commit
status (context = ``PR_FEEDBACK__GATE_STATUS_CONTEXT``) to block merge until a
``/feedback`` rating arrives. If the gate is later disabled, those already-pending
MRs would stay blocked. This tool scans open MRs, finds ones still ``pending`` on
our context, and (optionally) flips them to ``success`` to release them.
...
Auth/config come from environment variables (matching the running container):
    ...
    PR_FEEDBACK__GATE_STATUS_CONTEXT    gate context name (default pr-agent/feedback)

Usage (inside the pr-agent container, where env vars are set):
    python scripts/feedback_gate_sweep.py                 # dry-run, all member projects
    python scripts/feedback_gate_sweep.py --apply         # unlock all stuck MRs
    python scripts/feedback_gate_sweep.py --projects a/b c/d --apply
    python scripts/feedback_gate_sweep.py --all-projects --apply   # scan every visible project
"""
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Detect/unlock MRs stuck on the feedback gate.")
    ...
    ctx = _env("PR_FEEDBACK__GATE_STATUS_CONTEXT", "pr-agent/feedback")
